...
def main() -> int:
    ...
    image = f"{os.environ['HLA_TOOLKIT_IMAGE_NAME']}:{os.environ['HLA_TOOLKIT_IMAGE_TAG']}"
    dockerfile_dir = Path(__file__).parent
    dockerfile_path = dockerfile_dir / os.environ["HLA_TOOLKIT_DOCKERFILE_NAME"]

    print(f"Building {image}...")
    result = subprocess.run(
        ["docker", "build", "-f", str(dockerfile_path), "-t", image, str(dockerfile_dir)],
    )
