...
#
# Test Linux:
#    docker run --rm hla_toolkit:v1 samtools --version | head -3
#    docker run --rm hla_toolkit:v1 spechla -h 2>&1 | head -5
#    docker run --rm hla_toolkit:v1 which spechla-extract-hla-reads

#    docker run --rm hla_toolkit:v1 bash -c "conda run -n spechla_env conda list spechla | grep -E '^spechla\s+1\.0\.10\b' || (echo 'SpecHLA version mismatch' && exit 1)"
#    docker run --rm hla_toolkit:v1 bash -c "samtools --version | head -3 && samtools --version | grep -qi 'libcurl' || (echo 'samtools missing libcurl support' && exit 1)"
#    docker run --rm hla_toolkit:v1 bash -c "which spechla && which spechla-extract-hla-reads && test -d \"\${CONDA_PREFIX}/share/spechla/db\" && test -d \"\${CONDA_PREFIX}/share/spechla/script\""
#
# Test Windows:
#    docker run --rm hla_toolkit:v1 bash -c "samtools --version | head -3"
#    docker run --rm hla_toolkit:v1 bash -c "spechla -h 2>&1 | head -5"
#    docker run --rm hla_toolkit:v1 which spechla-extract-hla-reads
